...
async def control_midea_ac(
    ...
    Example:
        ...
        # 设置温度24度，开启上下摆风
        result = control_midea_ac(device_id=12345678, temperature=24, swing_ud=1)

        # 设置节能模式
        result = control_midea_ac(device_id=12345678, preset_mode="eco")

        # 开启电辅热和防直吹
        result = control_midea_ac(device_id=12345678, aux_heat=1, prevent_straight_wind=1)
    """
    cloud = await _get_cloud_client()
    if not cloud:
    ...
    if preset_mode is not None:
        ...
        control_desc.append(f"预设模式={preset_mode}")
    
    if aux_heat is not None:
        if aux_heat not in (0, 1):
            return "error:invalid_aux_heat"
        control["Ptc"] = aux_heat
        control_desc.append(f"电辅热={'开' if aux_heat else '关'}")
    
    if dry is not None:
        if dry not in (0, 1):
            return "error:invalid_dry"
        control["Dry"] = dry
        control_desc.append(f"干燥模式={'开' if dry else '关'}")
    ...
    if prevent_straight_wind is not None:
        ...
        control_desc.append(f"防直吹={'开' if prevent_straight_wind else '关'}")
